var Formatter = require('./formatter').Formatter
  ...
  , readOnlyEnumerablePropertyFunction = require('./utils').readOnlyEnumerablePropertyFunction
...
var Runner = function(configuration) {
  ...
  readOnlyEnumerablePropertyFunction(this, "files", function() {
    var testFiles = [];

    for(var name in tests) {
      testFiles = testFiles.concat(tests[name].files);
    }
  ...
  var applyPreRunOperations = function(callback) {
    // Signal pre run for all plugins
    executePluginsEventMethod(plugins, 'onPreRun', [self]);

    // Setup the on exit listener
    process.on("exit", 
       function() {
        executePluginsEventMethod(plugins, 'onExit', []);
       });
  ...
  this.run = function(config_name, options) {  
    ...
    applyPreRunOperations(function(err) {
      ...
      // Add plugins to the test suite options
      test_suite_options.plugins = plugins;
