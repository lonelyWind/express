var TestControl = require('./test_control').TestControl
  ...
  , executePluginsEventMethod = require('./utils').executePluginsEventMethod;
  ...
  this.execute_parallel = function(config_name, options, callback) {
    ...
    this.configuration.createAndStart(config_name
      , { number_of_contexts: number_of_contexts, 
          filters: runner.filters }, function(err, configurations) {
      // Start configuration
      executePluginsEventMethod(options.plugins, 'onTestSuiteStart', [configurations, runner])
      ...
      var done = function(err) {
        ...
        // Done running parallel context
        if(number_of_contexts_left == 0) {
          // Start configuration
          executePluginsEventMethod(options.plugins, 'onTestSuiteEnd', [configurations])
          // Return the callback
          callback();
        }
      }
      ...
      if(self.options.schedulerHints != null) {
        ...
        for(var i = 0; i < self.files.length; i++) {
          ...
          for(var name in test) {
            var skip = false;
            // If we have filter functions, let's run them all, only
            // add the tests if all of them return true
            if(runner.filters.length > 0) {
              for(var k = 0; k < runner.filters.length; k++) {
                if(runner.filters[k].filter(test[name]) == false) {
                  skip = true;
                  break;
                }
              }
            ...
            // If we only want to run a single file
            if(typeof options.file == 'string' && self.files[i].indexOf(options.file) == -1 && !skip) {
              skip = true;
            }

            // If we are only doing a specific test
            if(typeof options.test == 'string' && name != options.test && !skip) {
              skip = true;
            }

            // Load balance the tests
            if(skip == false) {
              if(options.test && options.test == name) {
...
var process_parallel_tests_serially = function(self, configuration, tests, options, callback) {
  ...
  var done_function = function(_test_control) {
    return function() {
      // Test is done
      executePluginsEventMethod(options.plugins, 'onTestEnd', [_test_control]);
      ...
      configuration.teardown(function() {
        ...
        processor(function() {
          self.runner.emit("test_done", _test_control, configuration);
        })
  ...
  configuration.setup(function() {
    // Test is done
    executePluginsEventMethod(options.plugins, 'onTestStart', [test, self.runner]);
